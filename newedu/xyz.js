(function() {
  ...
  /*
    Initialize with ttp.init( { key: '0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc' } )
      OR!
    Initialize with ttp.init( { key: 'https://docs.google.com/spreadsheet/pub?hl=en_US&hl=en_US&key=0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc&output=html&widget=true' } )
      OR!
    Initialize with ttp.init('0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc')
  */

  var ttp = function(options) {
    // Make sure ttp is being used as a constructor no matter what.
    if(!this || !(this instanceof ttp)) {
      return new ttp(options);
    }
    ...
    if (this.singleton) {
      if (typeof(ttp.singleton) !== 'undefined') {
        this.log('WARNING! ttp singleton already defined');
      }
      ttp.singleton = this;
    }
    ...
    if (!this.key) {
      this.log('You need to pass ttp a key!');
      return;
    }
  ...
  ttp.init = function(options) {
    return new ttp(options);
  };

  ttp.sheets = function() {
    this.log('Times have changed! You\'ll want to use var ttp = ttp.init(...); ttp.sheets(...); instead of ttp.sheets(...)');
  };
  ...
  ttp.prototype = {
    ...
    */
    injectScript: function(path, callback) {
      ...
      if (this.singleton) {
        if (callback === this.loadSheets) {
          callbackName = 'ttp.singleton.loadSheets';
        } else if (callback === this.loadSheet) {
          callbackName = 'ttp.singleton.loadSheet';
        }
      } else {
        var self = this;
        callbackName = 'tt' + (+new Date()) + (Math.floor(Math.random()*100000));
        // Create a temp callback which will get removed once it has executed,
        // this allows multiple instances of ttp to coexist.
        ttp.callbacks[ callbackName ] = function () {
          var args = Array.prototype.slice.call( arguments, 0 );
          callback.apply(self, args);
          script.parentNode.removeChild(script);
          delete ttp.callbacks[callbackName];
        };
        callbackName = 'ttp.callbacks.' + callbackName;
      }
    ...
    /*
      This will only run if ttp is being run in node.js
    */
    ...
    */
    ...
    /*
      ...
      if simpleSheet === true, then don't return an array of ttp.this.models,
    ...
    */
    ...
    /*
      Load all worksheets of the spreadsheet, turning each into a ttp Model.
    ...
    */
    ...
    /*
      Parse a single list-based worksheet, turning it into a ttp Model
  ...
  /*
    ttp.Model stores the attribute names and parses the worksheet data
      to turn it into something worthwhile

    Options should be in the format { data: XXX }, with XXX being the list-based worksheet
  */
  ttp.Model = function(options) {
    var i, j, ilen, jlen;
    this.columnNames = [];
    ...
    if (typeof(options.data.feed.entry) === 'undefined') {
      options.ttp.log('Missing data for ' + this.name + ', make sure you didn\'t forget column headers');
  ...
  ttp.Model.prototype = {
    ...
    */
    ...
    fetchPrettyColumns: function() {
      ...
      var that = this;
      this.ttp.requestData(cellurl, function(data) {
        that.loadPrettyColumns(data);
      });
    },

    beforeReady: function() {
      if(this.ttp.postProcess) {
        for (var i = 0, ilen = this.elements.length; i < ilen; i++) {
          this.ttp.postProcess(this.elements[i]);
        }
      }
    },
